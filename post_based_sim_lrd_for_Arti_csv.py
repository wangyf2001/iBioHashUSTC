...
def main():
    ...
    for feat_dir, weight in feat_dir_dict.items():
        ...
        if mode_split == 'method1':  # 分组后，组内top必须严格相同
            ...
            #         skip_i.extend([i, int(top4_index[1]), int(top4_index[2]), int(top4_index[3])])  # 包含则之后跳过处琄1�71ￄ1�77
            ...
            # print(group_2)  # 有近1837组，3674个query＄1�71ￄ1�7736740个gallery
            # 可解冄1�71ￄ1�774574个query，剩佄1�71ￄ1�775426个各戄1�71ￄ1�771组，只能使用剩余的1�71ￄ1�7747260个gallery，明显不够�ￄ1�71ￄ1�77
            # 扢�以需要放宽条件，改为＄1�71ￄ1�775丄1�71ￄ1�771组，只要互相之间朄1�71ￄ1�774个相同即可；4个一组，只要互相朄1�71ￄ1�773个相同即可；3个一组，只要互相朄1�71ￄ1�772个相同即可；2个一组，彼此互为top1
            ...
            # 5:929组，4645个query，需18580个gallery＄1�71ￄ1�77
            # 4＄1�71ￄ1�77193组，772个query，需3860个gallery＄1�71ￄ1�77
            # 3＄1�71ￄ1�77413组，1239个query，需8260个gallery＄1�71ￄ1�77
            # 2＄1�71ￄ1�77503组，406个query，需4060个gallery＄1�71ￄ1�77
            # 1＄1�71ￄ1�772938组，2938个query，需58760个gallery＄1�71ￄ1�77 共需93550个gallery，有1w的缺口，但大致ok
            ...
            for topk_select in range(10, 5, -1):  
                ...
                for i in range(len(sorted_index)):
                    ...
                    public_subset = []
                    for el in elements: 
                        if elements[el] >= topk_select - 2: # 如topk_select=10，若某个query出现8次，则认为属于公共子雄1�7
                            public_subset.append(el)
                    
                    if len(public_subset) < topk_select - 2: # 如topk_select=10，若公共子集长度小于8，则认为质量不太髄1�7
                        continue
            ...
            for i in range(2, len(group_select)):
                ...
                print("{}: {}组1�7, {}个query, 霢�{}个gallery".format(i, len(group_select[i]), i * len(group_select[i]), 50 * len(group_select[i])))
            print("1: 1组1�7, {}个query, 霢�{}个gallery".format(9755 - stat_q_num, 50*(9755 - stat_q_num)))
            ...
            #     sim_among_group = [[] for i in range(9)]  # 记录单个query和每组的平均相似庄1�7
            #     for group_topk_select_i in range(len(group_select)): # 先从2组开始，丢�直遍历到10组1�7
            ...
            #     # 遍历 sim_among_group，寻找相似度朢�大的组所在的位置
            ...
            # dis = -(dis-1) # 将相似度转换为距禄1�71ￄ1�77
            # model = model.fit(dis.cpu().numpy())
            # # 0 的最相似应为 0, 1228, 2216, 3822, 3855
            # # for i in range(9755):
            # #     if model.labels_[i] == model.labels_[0]: print(i)
            # # 经检查，效果很不稳定，簇大的大，小的射1�71ￄ1�77

            # 谱聚籄1�71ￄ1�77
            model = SpectralClustering(affinity = 'precomputed', n_clusters=1000,)
            dis = dis + 2
            model = model.fit(dis.cpu().numpy())
            # 0 的结果是＄1�71ￄ1�770, 468, 1228, 1605, 2174, 2216, 3822, 3855, 4759, 5522。效果还可以, 但人工对比后，也不是特别靠谱感觉，后期可以尝试，10min内出结果〄1�71ￄ1�77
            print("-")

        # ======= gallery 分配 =======
        qg_dis, qg_sorted_index = metric(query_fea, gallery_fea) # + 2
        qg_sorted_index_assign = torch.zeros((9755, 30), dtype=int)
        used_gallery_index = []
        
        # group_topk_select = group_select[0]
        for group_idx in range(9): # topk 5-2, # 调换分配顺序，先分配10组，朢�后分酄1�72组1�7
            ...
            for group_i in range(len(group_topk_select)):
                ...
                #         #     current_gallery_point[q_index_i] += 1  # query对应gallery的指钄1�71ￄ1�77 + 1
                ...
                # 方法丢�＄1�7 取一组内不重复的朢�相似的gallery；方法二：取出现次数朢�多的gallery
                # 方法丢�
                while len(used_gallery_index_pergroup) != 30:
                    for q_index_i in range(len(group_topk_select[group_i])):
                        ...
                        while cur_g_index in used_gallery_index:  # gallery已分配过，从总的里面眄1�7
            ...
            for sim_i in range(len(sim_among_group)):
                for sim_j in range(len(sim_among_group[sim_i])):
                    if sim_among_group[sim_i][sim_j] > max_sim:
                        ...
                        max_i = sim_i
                        max_j = sim_j
            # 如果max_sim>=交集数量阈��1�7, 记录要加入的组1�7
            if max_sim >= sim_cnt_thr:
                re_assign_single_q[i] = (max_i, max_j)
        
        print('单个query在交集数量阈倄1�7={}下，成功合并的数釄1�7={}'.format(sim_cnt_thr, len(re_assign_single_q)))
        # 将单个query合并刄1�72及以上组 
        for single_q in re_assign_single_q:
            re_assign_i, re_assign_j = re_assign_single_q[single_q]
            group_select[re_assign_i][re_assign_j].append(single_q)
        ...
        for i in range(2, len(group_select) + 2):
            for j in range(len(group_select[i - 2])):
                cur_group = str(i) + '_' + str(j)
                for img_idx in group_select[i - 2][j]:
                    groups_dict[query_names[img_idx]] = cur_group

        for i in range(9755):
            if i in skip_i:
                continue
            else:
                groups_dict[query_names[i]] = 'single'
        groups = []
        for idx, name in enumerate(query_names):
            groups.append(groups_dict[query_names[idx]])
        
        assert len(groups) == len(query_names)

        result_dict = pd.DataFrame({'Id':query_names,'Group':groups})
        result_dict[['Id','Group']].to_csv(os.path.join('/home/data1/changhao/iBioHash/Datasets', 'query_group_sim_post.csv'), index=False)
        ...
        # print('单query有{}丄1�7'.format(single_q_cnt))
