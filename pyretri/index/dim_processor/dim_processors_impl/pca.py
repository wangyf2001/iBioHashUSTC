...
class PCA(DimProcessorBase):
    ...
    def _train(self, fea_dir: str) -> None:
        ...
        """
        # train_fea, _ = feature_loader.load(fea_dir, self.feature_names)
        query_fea,gallery_fea, query_names,gallery_names = feature_loader.load(fea_dir)
        
        
        if self._hyper_params["l2"]:
            gallery_fea = normalize(gallery_fea, norm="l2")
        self.pca.fit(gallery_fea)
