...
def parse_args():
    ...
    parser.add_argument('--config_file', '-cfg', default='/home/data1/changhao/iBioHash/Codes/pytorch-image-models-main/post_config/market_w_tricks_zgp.yaml', metavar='FILE', type=str, help='path to config file')
    parser.add_argument('--feat_dir', '-fd', default='/home/data1/changhao/iBioHash/Results/features/eva_metaformer_90000_woQ_e4', metavar='FILE', type=str, help='path to feature')
...
def main():
    ...
    for feat_dir, weight in feat_dir_dict.items():
        ...
        # 保存输出文件
        # output_file_path = os.path.join('/home/data1/zgp/mmsport/fea/vit_l_openai_colorjiter_fea', args.domain, "{}_dba{}_qe{}".format(feat_dir.split('/')[-1], cfg.index.feature_enhancer.DBA.enhance_k, cfg.index.re_ranker.QE.qe_k))
        output_file_path="/home/data1/zgp/mmsport/fea/vit_l_openai_colorjiter_fea"
        ...
        with open(os.path.join(output_file_path,'gallery_pdq_768.pkl'),'wb') as f:
            pickle.dump(gallery_fea.cpu().numpy(), f)

        with open(os.path.join(output_file_path,'query_pdq_768.pkl'),'wb') as f:
