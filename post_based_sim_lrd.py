...
feat_dir_dict = {
    "/home/data1/changhao/iBioHash/Results/features/maxvit_xlarge_fr2_e86": 1,
    }
...
def main():
    ...
    for feat_dir, weight in feat_dir_dict.items():
        ...
        # load names for different server
        # _, _, query_names, gallery_names = feature_loader.load('/home/data1/changhao/iBioHash/Results/features/maxvit_fr2_same_domain_e49')
        ...
        if mode_split == 'method1':  # 分组后，组内top必须严格相同
            ...
            for topk_select in range(10, 5, -1):   # 从10个一组开始寻找，找组内的公共子集，
                ...
                for i in range(len(sorted_index)):
                    ...
                    elements = dict()  # 统计该组query出现次数
                    temp_index.append(topk_index.sort().values) # 把自己的也加进去
                    for ti in range(len(temp_index)):
                        for q_temp in temp_index[ti]:
                            q_temp = int(q_temp)
                            elements[q_temp] = elements.get(q_temp, 0) + 1
        ...
        result_dict[['Id','Predicted']].to_csv(os.path.join('/home/data1/changhao/iBioHash/Codes/pytorch-image-models-main/lrd_post_sim_submit', 'submit_post_based_sim_method21_maxvit_thr5_top40.csv'), index=False)
